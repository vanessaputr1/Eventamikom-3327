...
@php
    $routePrefix = request()->routeIs('organizer.*') ? 'organizer' : 'admin';
@endphp
...
<div class="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm max-w-4xl">
    <form action="{{ route($routePrefix.'.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
